    ...
    <div style=${{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style=${{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px 14px', background: C.bg2, borderBottom: `1px solid ${C.bdr}`, flexShrink: 0 }}>
        ...
        <div style=${{ flex: 1, minWidth: 0 }}>
          <p style=${{ margin: 0, ...T.h2 }}>HƯỚNG DẪN TẬP LUYỆN</p>
          <p style=${{ margin: '1px 0 0', fontFamily: F.serif, fontStyle: 'italic', fontSize: 11.5, color: C.txt3, whiteSpace: 'nowrap' }}>Không biết bắt đầu từ đâu? Bắt đầu ở đây.</p>
        </div>
      </div>

      <div style=${{ flex: 1, overflowY: 'auto', padding: '16px 16px 40px' }}>
        ${groups.length === 0
          ? html`<${Empty} icon="book" msg="Chưa có bài hướng dẫn" sub="Thư viện đang được bổ sung"/>`
          : groups.map(grp => {
            const a = actOf(grp.type);
            return html`
              <div key=${grp.type} style=${{ marginBottom: 20 }}>
                <div style=${{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 10 }}>
                  ...
                  <p style=${{ margin: 0, fontFamily: F.display, fontWeight: 700, fontSize: 15, letterSpacing: '.08em', color: C.txt1, textTransform: 'uppercase' }}>${a.label}</p>
                </div>
