...
export function AnnouncementFeed({ parent, canPost, me, isAdmin }) {
  ...
  const post = async () => {
    const t = text.trim();
    if ((!t && !imgFile) || busy) return;
    setBusy(true);
    try {
      const created = await addAnnouncement(parent, me, t, imgFile);
      setPosts(p => [{ ...created, createdAt: { toDate: () => new Date() } }, ...p]);
      setText(''); clearImg();
    } catch (e) { window.alert('Đăng thông báo thất bại. Thử lại nhé.'); }
  ...
  const saveEdit = async (oldPost) => {
    ...
    if ((!t && !eImgFile && !willKeep) || busy) return;
    setBusy(true);
    try {
      const updated = await editAnnouncement(parent, me, oldPost, t, eImgFile, willKeep);
      setPosts(list => list.map(x => x.id === oldPost.id ? { ...updated, createdAt: { toDate: () => new Date() } } : x));
      cancelEdit();
    } catch (e) { window.alert('Sửa thông báo thất bại. Thử lại nhé.'); }
    finally { setBusy(false); }
  };
    ...
    <div style=${{ background: '#fff', border: `1px solid ${C.bdr}`, borderRadius: r.lg, overflow: 'hidden', marginBottom: 12 }}>
          ...
          ${canPost && html`
            <div style=${{ marginBottom: 12 }}>
              <div style=${{ display: 'flex', gap: 8 }}>
                ...
                <button onClick=${post} class="btn-action" style=${{ alignSelf: 'flex-end', background: ACC, border: 'none', borderRadius: r.md, padding: '9px 14px', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: (text.trim() || imgFile) && !busy ? 1 : 0.5 }}>Đăng</button>
              </div>
              ${imgPreview && html`
              ...
              ${!imgPreview && html`
                <label style=${{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 8, cursor: 'pointer', fontSize: 12.5, color: C.txt3 }}>
                  ...
                  <span style=${{ fontSize: 15 }}>📷</span> Thêm ảnh
                </label>`}
            </div>`}
          ${!loaded
      ...
      : posts.length === 0
        ...
        : html`<label style=${{ display: 'inline-flex', alignItems: 'center', gap: 6, marginTop: 8, cursor: 'pointer', fontSize: 12.5, color: C.txt3 }}>
                        ...
                        <span style=${{ fontSize: 15 }}>📷</span> ${eRemoveImg ? 'Đã gỡ ảnh — thêm ảnh mới' : 'Thêm ảnh'}
                      </label>`}
                  <div style=${{ display: 'flex', gap: 8, marginTop: 10 }}>
                    <button onClick=${cancelEdit} class="btn-action" style=${{ flex: 1, padding: '8px', borderRadius: r.md, border: `1px solid ${C.bdr}`, background: '#fff', color: C.txt2, fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>Huỷ</button>
                    <button onClick=${() => saveEdit(p)} class="btn-action" style=${{ flex: 1, padding: '8px', borderRadius: r.md, border: 'none', background: ACC, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: busy ? 0.6 : 1 }}>Lưu</button>
                  </div>
                </div>`
          : html`
                <div key=${p.id} style=${{ padding: '10px 0', borderTop: `1px solid ${C.bdr}` }}>
                  <div style=${{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                    ...
                    ${canManage(p) && html`<button onClick=${() => openEdit(p)} class="btn-action" style=${{ background: 'none', border: 'none', color: ACC, fontSize: 11, cursor: 'pointer', padding: 0 }}>Sửa</button>`}
                    ${canManage(p) && html`<button onClick=${() => del(p)} class="btn-action" style=${{ background: 'none', border: 'none', color: C.txt3, fontSize: 11, cursor: 'pointer', padding: 0 }}>Xoá</button>`}
                  </div>
