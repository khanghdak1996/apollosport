...
export function GoalForm({ scope, clubId, clubName, clubSport, me, onCreated, onClose }) {
  ...
  const metricOpts = [{ v: 'sessions', l: t('gf.metricSessions') }, { v: 'minutes', l: t('gf.metricMinutes') }, ...(allowKm ? [{ v: 'distanceKm', l: t('gf.metricKm') }] : [])];
  const unit = metric === 'minutes' ? t('goal.mMinutesU') : metric === 'distanceKm' ? t('goal.mKmU') : t('goal.mSessionsU');
  ...
  return html`
    <div style=${{ position: 'absolute', inset: 0, zIndex: 250, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'flex-end' }} onClick=${() => !saving && onClose()}>
      <div onClick=${e => e.stopPropagation()} style=${{ width: '100%', background: C.bg1, borderRadius: '20px 20px 0 0', padding: '18px 16px calc(18px + env(safe-area-inset-bottom))', maxHeight: '90%', overflowY: 'auto' }}>
        <p style=${{ margin: '0 0 16px', ...T.h2 }}>${scope === 'club' ? t('gf.clubTitle') + (clubName ? ` · ${clubName}` : '') : t('gf.companyTitle')}</p>

        <p style=${{ margin: '0 0 6px 2px', ...T.label }}>${t('gf.nameLabel')}</p>
        <input value=${title} onInput=${e => setTitle(e.target.value)} maxLength=${80} placeholder=${t('gf.namePlaceholder')} style=${{ ...input, marginBottom: 14 }}/>

        <p style=${{ margin: '0 0 8px 2px', ...T.label }}>${t('gf.measure')}</p>
      ...
      return html`<button key=${o.v} onClick=${() => setMetric(o.v)} class="btn-action" style=${{ flex: 1, padding: '10px', borderRadius: r.md, cursor: 'pointer', fontSize: 12.5, fontWeight: 600, border: `1px solid ${on ? BRAND.blue : C.bdr}`, background: on ? BRAND.blue : C.bg2, color: on ? '#fff' : C.txt2 }}>${o.l}</button>`;
        ...
        ${scope === 'company' && html`
          <p style=${{ margin: '0 0 8px 2px', ...T.label }}>${metricNeedsSport ? t('gf.sportRequired') : t('gf.sportOptional')}</p>
          <div style=${{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
            <button onClick=${() => setSport('')} class="btn-action" style=${{ padding: '7px 12px', borderRadius: r.pill, cursor: 'pointer', fontSize: 12.5, fontWeight: 600, border: `1px solid ${sport === '' ? BRAND.blue : C.bdr}`, background: sport === '' ? BRAND.blue : C.bg2, color: sport === '' ? '#fff' : C.txt2, opacity: metricNeedsSport ? 0.4 : 1, pointerEvents: metricNeedsSport ? 'none' : 'auto' }}>${t('gf.allSports')}</button>
            ${ACTIVITIES.map(x => {
      const on = sport === x.id;
      return html`<button key=${x.id} onClick=${() => setSport(x.id)} class="btn-action" style=${{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: r.pill, cursor: 'pointer', fontSize: 12.5, fontWeight: 600, border: `1px solid ${on ? BRAND.blue : C.bdr}`, background: on ? BRAND.blue : C.bg2, color: on ? '#fff' : C.txt2 }}><${SportIcon} k=${x.iconKey} size=${15} color=${on ? '#fff' : sportColor(x.iconKey)}/> ${actLabel(x.id)}</button>`;
    })}
          </div>`}
        ${scope === 'club' && clubSport && html`<p style=${{ margin: '0 0 14px 2px', fontSize: 12, color: C.txt3, display: 'flex', alignItems: 'center', gap: 5 }}>${t('gf.sportPrefix')} <${SportIcon} k=${actOf(clubSport).iconKey} size=${14} color=${sportColor(actOf(clubSport).iconKey)}/> <strong style=${{ color: C.txt2 }}>${actLabel(clubSport)}</strong> ${t('gf.fromClub')}</p>`}

        <p style=${{ margin: '0 0 6px 2px', ...T.label }}>${t('gf.targetLabel', { u: unit.toUpperCase() })}</p>
        <input type="number" inputMode="numeric" value=${target} onInput=${e => setTarget(e.target.value)} placeholder=${t('gf.targetPlaceholder')} style=${{ ...input, marginBottom: 14 }}/>

        <div style=${{ display: 'flex', gap: 10, marginBottom: 18 }}>
          <div style=${{ flex: 1 }}>
            <p style=${{ margin: '0 0 6px 2px', ...T.label }}>${t('gf.start')}</p>
            <input type="date" value=${start} onInput=${e => setStart(e.target.value)} style=${{ ...input, padding: '10px 12px', fontSize: 14 }}/>
          </div>
          <div style=${{ flex: 1 }}>
            <p style=${{ margin: '0 0 6px 2px', ...T.label }}>${t('gf.end')}</p>
            <input type="date" value=${end} onInput=${e => setEnd(e.target.value)} style=${{ ...input, padding: '10px 12px', fontSize: 14 }}/>
          </div>
        </div>

        <div style=${{ display: 'flex', gap: 8 }}>
          <button onClick=${onClose} class="btn-action" style=${{ flex: 1, padding: '12px', borderRadius: r.md, border: `1px solid ${C.bdr}`, background: C.bg2, color: C.txt2, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>${t('common.cancel')}</button>
          <${Btn} onClick=${submit} cx=${{ flex: 1, opacity: (!valid || saving) ? 0.5 : 1, pointerEvents: (!valid || saving) ? 'none' : 'auto' }}>${saving ? t('gf.creating') : t('gf.create')}</${Btn}>
        </div>
      </div>
    </div>`;
}
