...
import { actOf, fieldsOf, rpeOf, RPE_LEVELS, rpeLabel, rpeDesc, actLabel, flabel } from '../domain/activities.js';
...
export function LogActivity({ type, defaultVisibility = 'company', onBack, onSave, hasGuide = false, onOpenGuide }) {
  ...
  const effortLabel = rpeLabel(vals.rpe);
  ...
  const liveSubtitle = `${Math.round(durMin)} ${t('unit.min')}${paceStr && paceStr !== '—' ? ` · ${paceStr}` : ''} · ${effortLabel} · ~${liveMet} MET`;
  ...
  const renderRow = (f, i) => {
    if (f.type === 'pace') {
      return rowWrap(i, html`
        <div style=${{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style=${{ flex: 1, minWidth: 0 }}><p style=${{ margin: 0, ...T.label }}>${flabel(f)}</p></div>
    ...
    if (f.type === 'rpe') {
      return rowWrap(i, html`
        <p style=${{ margin: '0 0 2px', ...T.label }}>${t('rpe.field.label')}</p>
    ...
    if (f.type === 'seg' || f.type === 'select') {
      return rowWrap(i, html`
        <p style=${{ margin: '0 0 8px', ...T.label }}>${flabel(f)}</p>
    ...
    if (f.type === 'counter') {
      return rowWrap(i, html`
        <div style=${{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <p style=${{ margin: 0, flex: 1, ...T.label }}>${flabel(f)}</p>
    ...
    return rowWrap(i, html`
      <div style=${{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style=${{ flex: 1, minWidth: 0 }}>
          <p style=${{ margin: 0, ...T.label }}>${flabel(f)}</p>
          ${f.required ? html`<p style=${{ margin: '2px 0 0', fontSize: 11, color: C.txt5 }}>${t('common.required')}</p>` : ''}
        </div>
        ${f.type === 'time'
  ...
  return html`
    <div style=${{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      <div style=${{ padding: '12px 16px', borderBottom: `1px solid ${C.bdr}`, background: C.bg2, display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <p style=${{ flex: 1, margin: 0, fontFamily: F.display, fontWeight: 700, fontSize: 20, letterSpacing: '.03em', color: C.txt1, textTransform: 'uppercase', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>${actLabel(type)}</p>
        <button onClick=${submit} class="btn-action" style=${{ background: BRAND.blue, border: 'none', borderRadius: r.md, padding: '10px 18px', fontFamily: F.display, fontWeight: 700, fontSize: 15, letterSpacing: '.08em', color: '#fff', cursor: 'pointer', textTransform: 'uppercase', flexShrink: 0, opacity: (!canSave || saving) ? 0.5 : 1, pointerEvents: (!canSave || saving) ? 'none' : 'auto' }}>${saving ? '…' : t('common.save')}</button>
      </div>

      <div style=${{ flex: 1, overflowY: 'auto', padding: '16px 16px 40px', WebkitOverflowScrolling: 'touch' }}>
        <input value=${title} onInput=${e => setTitle(e.target.value)} placeholder=${t('log.titlePlaceholder', { sport: actLabel(type) })} style=${{ width: '100%', boxSizing: 'border-box', border: 'none', background: 'transparent', fontSize: 20, fontWeight: 600, color: C.txt1, marginBottom: 14, padding: 0 }}/>
        ...
        ${hasGuide && onOpenGuide && html`
          <button onClick=${onOpenGuide} class="btn-action" style=${{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', textAlign: 'left', cursor: 'pointer', background: C.bg3, border: 'none', borderRadius: r.md, padding: '11px 14px', marginBottom: 14 }}>
            ...
            <span style=${{ flex: 1, fontSize: 13, fontWeight: 600, color: BRAND.blue }}>${t('log.guideLink', { sport: actLabel(type) })}</span>
        ...
        ${hasMore && html`
          <button onClick=${() => setShowMore(s => !s)} class="btn-action" style=${{ display: 'flex', alignItems: 'center', gap: 6, background: 'transparent', border: 'none', color: BRAND.blue, fontSize: 13.5, fontWeight: 600, cursor: 'pointer', padding: '0 0 12px' }}>
            ${showMore ? t('log.less') + ' ▴' : t('log.more') + ' ▾'}
          </button>`}
        ...
        ${showMore && html`
          ...
          ${a.laps && html`
            <div style=${{ marginBottom: 14 }}>
              <p style=${{ margin: '0 0 8px 2px', ...T.label }}>${t('log.laps')}</p>
              ${laps.map((lp, i) => html`
              ...
              <button onClick=${addLap} class="btn-action" style=${{ width: '100%', padding: '10px', borderRadius: r.md, border: `1.5px dashed ${C.bdr2}`, background: 'transparent', color: C.txt2, fontSize: 13.5, cursor: 'pointer' }}>＋ ${t('log.addLap')}</button>
            </div>`}`}
        ...
        <div style=${{ display: 'flex', alignItems: 'center', gap: 12, background: BRAND.babyBlue, borderRadius: r.lg, padding: '14px 16px', marginBottom: 14 }}>
          <div style=${{ flex: 1, minWidth: 0 }}>
            ...
            <p style=${{ margin: '2px 0 0', fontFamily: F.serif, fontStyle: 'italic', fontSize: 12, color: '#3D6285' }}>${canSave ? liveSubtitle : (a.category === 'pace' ? t('log.needDistance') : t('log.needDuration'))}</p>
          </div>
        ...
        <p style=${{ margin: '0 0 6px 2px', ...T.label }}>${t('save.note.title')}</p>
        <textarea value=${note} onInput=${e => setNote(e.target.value)} placeholder=${t('save.note.placeholder')} rows=${2} style=${{ ...inputStyle, resize: 'none', marginBottom: 14 }}/>
        ...
        <label style=${{ display: 'block', cursor: 'pointer', marginBottom: 6 }}>
          ...
          ${photoPreview
            ...
            : html`<div style=${{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '18px', borderRadius: r.xl, border: `1.5px dashed #C7D8E6`, color: C.txt3, fontSize: 13, fontWeight: 600 }}><${SportIcon} k="photo" size=${18} color=${C.txt3}/> ${t('photo.add')}</div>`}
        </label>
        <p style=${{ margin: '0 0 16px 2px', fontSize: 11.5, color: C.txt4 }}>${t('photo.note')}</p>

        <p style=${{ margin: '0 0 8px 2px', ...T.label }}>${t('visibility.title')}</p>
        <div style=${{ display: 'flex', gap: 8 }}>
          ${[{ v: 'company', l: t('visibility.company'), k: 'globe' }, { v: 'private', l: t('visibility.private'), k: 'lock' }].map(o => {
