      ...
      <div key=${c.id} onClick=${() => onOpenClub(c.id)} class="card-hover" style=${{ display: 'flex', alignItems: 'center', gap: 12, background: C.bg2, border: `1px solid ${C.bdr}`, borderRadius: r.xl, padding: '13px 15px', marginBottom: 10, cursor: 'pointer' }}>
        ...
        <div style=${{ flex: 1, minWidth: 0 }}>
          ...
          <p style=${{ margin: '2px 0 0', fontSize: 12, color: C.txt3, display: 'flex', alignItems: 'center', gap: 5 }}>
            ${a.label} · ${c.memberCount || 0} thành viên${c.visibility === 'invite' ? html` · <${SportIcon} k="lock" size=${12} color=${C.txt4}/> riêng` : ''}
          </p>
        </div>
        ${joined && html`<span style=${{ fontSize: 11, fontWeight: 700, color: BRAND.blue, background: C.bg3, borderRadius: 20, padding: '4px 10px', flexShrink: 0, letterSpacing: '.02em' }}>Đã vào</span>`}
  ...
  return html`
    <${Wrap}>
      <div style=${{ padding: '12px 16px', borderBottom: `1px solid ${C.bdr}`, background: C.bg2, display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <p style=${{ margin: 0, flex: 1, ...T.h2 }}>CÂU LẠC BỘ</p>
        <button onClick=${() => setCreating(true)} class="btn-action" style=${{ display: 'flex', alignItems: 'center', gap: 5, background: BRAND.blue, border: 'none', borderRadius: 20, height: 34, padding: '0 14px', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
          <${SportIcon} k="plus" size=${15} color="#fff" sw=${2.4}/> Tạo nhóm
        </button>
      </div>
      ...
      : html`
            ${myClubs.length > 0 && html`
              <p style=${{ margin: '0 2px 9px', ...T.section }}>NHÓM CỦA BẠN</p>
              ${myClubs.map(clubCard)}
              <div style=${{ height: 14 }}/>`}
            ${otherClubs.length > 0 && html`<p style=${{ margin: '0 2px 9px', ...T.section }}>KHÁM PHÁ NHÓM</p>`}
      ...
      ${creating && html`
        <div style=${{ position: 'absolute', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'flex-end' }} onClick=${() => !saving && setCreating(false)}>
          <div onClick=${e => e.stopPropagation()} style=${{ width: '100%', background: C.bg1, borderRadius: '20px 20px 0 0', padding: '18px 16px calc(18px + env(safe-area-inset-bottom))', maxHeight: '88%', overflowY: 'auto' }}>
            <p style=${{ margin: '0 0 16px', ...T.h2 }}>TẠO CÂU LẠC BỘ</p>

            <p style=${{ margin: '0 0 6px 2px', ...T.label }}>TÊN NHÓM</p>
            <input value=${name} onInput=${e => setName(e.target.value)} maxLength=${60} placeholder="VD: CLB Chạy bộ Apollo" style=${{ width: '100%', boxSizing: 'border-box', padding: '11px 13px', borderRadius: r.md, border: `1px solid ${C.bdr}`, fontSize: 14.5, marginBottom: 14, background: C.bg2, color: C.txt1 }}/>

            <p style=${{ margin: '0 0 8px 2px', ...T.label }}>MÔN</p>
        ...
        return html`
                <button key=${a.id} onClick=${() => setSport(a.id)} class="btn-action" style=${{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', borderRadius: r.pill, cursor: 'pointer', fontSize: 12.5, fontWeight: 600, border: `1px solid ${on ? BRAND.blue : C.bdr}`, background: on ? BRAND.blue : C.bg2, color: on ? '#fff' : C.txt2 }}>
                  <${SportIcon} k=${a.iconKey} size=${15} color=${on ? '#fff' : sportColor(a.iconKey)}/> ${a.label}
                </button>`;
      })}
            </div>

            <p style=${{ margin: '0 0 8px 2px', ...T.label }}>QUYỀN THAM GIA</p>
            <div style=${{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ${[{ v: 'public', k: 'globe', l: 'Công khai', s: 'Ai cũng vào được' }, { v: 'invite', k: 'lock', l: 'Cần duyệt', s: 'Chủ nhóm duyệt' }].map(o => {
        ...
        return html`
            ...
            <p style=${{ margin: '0 0 6px 2px', ...T.label }}>MÔ TẢ (TUỲ CHỌN)</p>
            <textarea value=${desc} onInput=${e => setDesc(e.target.value)} rows=${2} maxLength=${300} placeholder="Nhóm này về điều gì?" style=${{ width: '100%', boxSizing: 'border-box', padding: '11px 13px', borderRadius: r.md, border: `1px solid ${C.bdr}`, fontSize: 14, marginBottom: 16, resize: 'vertical', fontFamily: 'inherit', background: C.bg2, color: C.txt1 }}/>

            <div style=${{ display: 'flex', gap: 8 }}>
              <button onClick=${() => setCreating(false)} class="btn-action" style=${{ flex: 1, padding: '12px', borderRadius: r.md, border: `1px solid ${C.bdr}`, background: C.bg2, color: C.txt2, fontSize: 14, fontWeight: 500, cursor: 'pointer' }}>Huỷ</button>
